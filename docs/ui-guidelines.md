...
The top-left 3×3 target cell shows the current external-field state through its icon and accepts explicit text drag-and-drop. The default DMO/RIS path is `mark in the source → Ctrl+C → import clipboard`; it does not inspect another process or silently consume stale clipboard data. UIA field capture is a developer-only test: normal releases keep it disabled, and `RADIMO_ENABLE_EXPERIMENTAL_UIA=1` plus a fresh session selection are required before it can run. The target cell's right-click menu offers the same clipboard action and, in the experimental UIA mode, can request marked text. Its small X clears the target or text source.
...
The Context panel also provides the read-only RIS Field Mapper as an experimental UIA diagnostic. In normal releases its controls return a local explanation and do not start UIA or PowerShell; a developer must launch with `RADIMO_ENABLE_EXPERIMENTAL_UIA=1` and select the experimental mode first. `Konfigurierte Felder lesen` reads only fields matching the configured label rules; `UIA-Felder prüfen` lists accessible text controls without reading their values. Matching uses accessible names, labels, help text, automation IDs, and parent container labels; class names alone are not sufficient. When no target is selected, the helper is hidden briefly before scanning so focus stays with the RIS. Exclusions are applied before value reads, and the result is grouped into named context sections. No scan runs at startup. The standalone Field Mapper uses the same gated UIA-only contract and has no AI or network path.
