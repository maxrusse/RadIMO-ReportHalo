...
Dictation stores the activated field's native control and returns to that control before inserting. It never selects all or moves to the end: the transcript is inserted at the current cursor/selection position, then the complete resulting field is refreshed as context for subsequent actions. If no UIA field is activated in the default clipboard mode, `Einsetzen` copies the transcript and asks the user to paste it at the RIS cursor. Other write actions continue to use their explicit replace or append contract.

The Context panel also provides the read-only RIS Field Mapper as an explicit UIA diagnostic. In clipboard mode its scan controls return a local explanation and do not start UIA or PowerShell; the user must select `UIA-Feldzugriff · testen` first. `Konfigurierte Felder lesen` reads only fields matching the configured label rules; `UIA-Felder prüfen` lists accessible text controls without reading their values. When no target is selected, the helper is hidden briefly before scanning so focus stays with the RIS. Exclusions are applied before value reads, and the result is grouped into named context sections. No scan runs at startup. The standalone Field Mapper uses the same UIA-only contract and has no AI or network path.
