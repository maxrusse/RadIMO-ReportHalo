...
The Context panel also provides the read-only RIS Field Mapper. `Kontext aus aktivem Fenster lesen` reads only fields matching the configured label rules; `Felder prüfen` lists all detected text controls without reading their values. Exclusions are applied before value reads, and the result is grouped into named context sections. The standalone Field Mapper uses the same contract and has no AI or network path.
