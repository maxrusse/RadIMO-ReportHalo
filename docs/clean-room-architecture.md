...
- `src/windows-field-bridge.js` exposes only the clipboard guard and the narrower opt-in UIA path in `src/windows-safe-field-bridge.js`; native code injection, keystroke replay, and execution-policy bypasses are not part of the product bridge. The default DMO/RIS source path is explicit clipboard transfer. `src/windows-field-mapper.js` owns the label-pattern profile, exclusion-first matching, and structured context serializer.
