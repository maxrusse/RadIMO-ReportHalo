...
- `src/windows-field-bridge.js` keeps the legacy Native-/PowerShell bridge behind an explicit compatibility mode; `src/windows-safe-field-bridge.js` provides the narrower opt-in UIA path without native code injection or an execution-policy bypass. The default DMO/RIS source path is explicit clipboard transfer. `src/windows-field-mapper.js` owns the label-pattern profile, exclusion-first matching, and structured context serializer.
