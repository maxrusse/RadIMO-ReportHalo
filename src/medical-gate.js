...
const MODE_PROMPTS = {
  ...
  correction: "LEKTORAT MODE: Correct only relevant spelling, grammar, punctuation, and clear dictation artifacts in the supplied text. Preserve existing headings and OPB when present. Do not add content, stylistically rewrite, or change medical meaning. Return every paragraph, heading, and unchanged passage as one complete text block in the text field; never return only changed excerpts, a summary, or an ellipsis, even if only one word changes. If no correction is needed, return an unchanged copy. Keep all change entries and visible medical or logical issues outside text as short notes, and do not correct those issues.",
